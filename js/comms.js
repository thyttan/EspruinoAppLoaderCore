    ...
    return Comms.write("\x03").then(result => {
      ...
      then((result) => new Promise((resolve, reject) => {
        ...
        if (Const.SINGLE_APP_ONLY) // only one app on device, info file is in app.info
          ...
          cmd = `\x10{${device}.print("[");let fs=require("fs");if (!fs.statSync("APPINFO"))fs.mkdir("APPINFO");fs.readdirSync("APPINFO").forEach(f=>{if (f.endsWith(".info")) try{let j=JSON.parse(fs.readFileSync("APPINFO/"+f))||"{}";${device}.print(JSON.stringify({id:f.slice(0,-5),version:j.version,files:j.files,data:j.data,type:j.type})+",")}catch{}});${device}.println(${finalJS})}\n`;
